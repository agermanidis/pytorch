...
class MemoryTracker:
    ...
    def show_traces(self, path: str = "") -> None:
        ...
        min_val = min(y_1 + y_2 + y_3) * 0.9
        max_val = max(y_1 + y_2 + y_3) * 1.1
        x = list(range(len(y_1)))
        #
        plt.figure(1)
        plt.plot(x, list(y_1), label="allocated_memory")
        plt.plot(x, list(y_2), label="active_memory")
        plt.plot(x, list(y_3), label="reserved_memory")
